  ...
  return (
    <>
      <FlatList
        ...
        renderItem={({ item }) => (
          <Card
            // style={{ maxWidth: 160 }}
            mode="contained"
            onPress={() => selectCurrentContact(item.name, item.phoneNumber)}
          >
            <Card.Content>
              <Text variant="displaySmall">{item.phoneNumber}</Text>
              <Text>{item.name}</Text>
            </Card.Content>
          </Card>
        )}
      ...
      <Portal>
        ...
        >
          <Dialog.Title>Anda yakin untuk melakukan panggilan ini?</Dialog.Title>
          <Dialog.Content>
            <Text>
              Anda akan menelpon ke layanan{" "}
              <Text style={{ fontWeight: "bold" }}>{selectedContact.name}</Text>
            </Text>
          </Dialog.Content>
          <Dialog.Actions>
            <Button
              ...
              icon={"phone"}
              onPress={() =>
                Linking.openURL(`tel:${selectedContact.phoneNumber}`)
              }
            >
              Panggil
            </Button>
            <Button
              style={{}}
              onPress={() => setContactConfirmationDialogVisible(false)}
            >
              Batal
            </Button>
          </Dialog.Actions>
